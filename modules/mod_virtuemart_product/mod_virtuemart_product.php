...
$currentCat = (int) JRequest::getVar('virtuemart_category_id');
...
$key = 'products'.$category_id.'.'.$max_items.'.'.$filter_category.'.'.$display_style.'.'.$products_per_row.'.'.$show_price.'.'.$show_addtocart.'.'.$Product_group.'.'.$virtuemart_currency_id.'.'.$currentCat;
...
if (!($output = $cache->get($key))) {
	...
	$productModel = VmModel::getModel('Product');
        ...
        if($Product_group != 'cat')
            $products = $productModel->getProductListing($Product_group, $max_items, $show_price, true, false,$filter_category, $category_id);
        else {
            $categoryModel = VmModel::getModel('Category');
            $category=$categoryModel->getCategory($currentCat,false);
            $products = $productModel->getProductListing ($group = FALSE, $nbrReturnProducts = 1000, $withCalc = TRUE, $onlyPublished = TRUE, $single = FALSE, $filterCategory = TRUE, $currentCat);
        }

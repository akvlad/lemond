 ...
 *
 */
class plgVmShipmentDoor extends vmPSPlugin {
    ...
    function plgVmOnShowOrderFEInfo ($virtuemart_order_id){
        ...
        $html='г.'.$shipinfo->city.', '.$shipinfo->adress.' '.
            (!empty($shipinfo->house) ? ', д. '.$shipinfo->house : '').
            (!empty($shipinfo->apart) ? ', кв. '.$shipinfo->apart : '').
            (!empty($shipinfo->ofice) ? ', оф. '.$shipinfo->ofice : '');
        return $html;
    }
	...
	function plgVmConfirmedOrder ($order_id, $order) {
		...
		//die('#1');
                ...
                $values=JRequest::getVar('doors_ship',null);
	...
	function getOrderShipmentHtml ($virtuemart_order_id) {
		...
		$html .= $this->getHtmlRowBE ('WEIGHT_COUNTRIES_SHIPPING_NAME', "Доставка до двери");
		$html .= $this->getHtmlRowBE ('Город', $shipinfo->city);
		$html .= $this->getHtmlRowBE ('Адрес', $shipinfo->adress);
		$html .= $this->getHtmlRowBE ('Дом', $shipinfo->house);
        $html .= $this->getHtmlRowBE ('Квартира', $shipinfo->apart);
        $html .= $this->getHtmlRowBE ('Офис', $shipinfo->ofice);
        $html .= $this->getHtmlRowBE ('Время', $shipinfo->from.' - '.$shipinfo->to);
		$html .= '</table>' . "\n";
	...
	public function plgVmDisplayListFEShipment (VirtueMartCart $cart, $selected = 0, &$htmlIn) {
                ...
                return $this->displayListFE ($cart, $selected, $htmlIn,3);
	}
        ...
        protected function getPluginHtml ($plugin, $selectedPlugin, $pluginSalesPrice) {
             $pluginmethod_id = $this->_idName;
                ...
                ob_start();
                include (JPATH_SITE.'/plugins/vmshipment/door/tmpl/default.php');
		$html = ob_get_clean();
                return $html;
        }

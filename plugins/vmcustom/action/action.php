...
class plgVmCustomAction extends vmCustomPlugin {
	...
	function plgVmOnProductEdit($field, $product_id, &$row,&$retValue) {
        ...
        $query='SELECT p.virtuemart_product_id as product_id, IF ( c.category_name IS NULL, p.product_name, CONCAT (c.category_name," - ",p.product_name) ) as product_name,
                pm.virtuemart_manufacturer_id as manufacturer
		...
		foreach($products as $product){
            if ($product->manufacturer == null) $product->manufacturer=0;
			$ProductOptions[]=JHTML::_('select.option', $product->product_id,
               $product->product_name, array('option.attr'=>'rel','attr'=> $product->manufacturer));
		}
		//var_dump($ProductOptions); die();

        $j='jQuery(document).ready(function(){jQuery("#plugin_param1actionproduct1").chosen().change(function(event) {
    var val=jQuery(event.target).val();
    var rel=jQuery("#plugin_param1actionproduct1 option[value="+val+"]").attr("mf");
    var opts=jQuery("#plugin_param1actionproduct2 option");
    opts.removeAttr("disabled");
    opts.removeAttr("selected");
    var checked=false;
    jQuery.each(opts,function(i,el){var _rel=jQuery(el).attr("mf"); if(_rel!=rel) jQuery(el).attr("disabled","true");
        else if(!checked) {jQuery(el).attr("selected","selected"); checked = true; } });
    jQuery("#plugin_param1actionproduct2").trigger("liszt:updated");
    checked = false;
});});';
        $html ='<div>';
        $html .='<script type="text/javascript">'.$j.'</script>';

		$html .='<p>Товар1: <select id="plugin_param1actionproduct1" name="plugin_param['.$row.']['.$this->_name.'][product1]" >';
        foreach($products as $product){

            if(isset($this->params->product1)){
            $checked=$this->params->product1 == $product->product_id ? 'selected="selected"' : '';}
            $html.='<option '.$checked.' value="'.$product->product_id.'" mf="'.$product->manufacturer.'">'.$product->product_name.'</option>';

        }
        $html.="</select>";

        $html .='<p>Товар2: <select id="plugin_param1actionproduct2" name="plugin_param['.$row.']['.$this->_name.'][product2]" >';
        foreach($products as $product){
            $checked=$this->params->product2 == $product->product_id ? 'selected="selected"' : '';
            $html.='<option '.$checked.' value="'.$product->product_id.'" mf="'.$product->manufacturer.'">'.$product->product_name.'</option>';
        }
        $html.="</select>";
        $html .='<input type="hidden" value="'.$this->virtuemart_custom_id.'" name="plugin_param['.$row.']['.$this->_name.'][virtuemart_custom_id]">';
		$html .= '<p>Старая цена: <input type="text" name="plugin_param['.$row.']['.$this->_name.'][old_price]" value='.$this->params->old_price.'>';
        $html .='</div>';
        //var_dump($html); die();
                //die($html);
		$retValue .=  $html  ;

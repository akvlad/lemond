...
class CurrencyDisplay {
	 ...
	 */
	public function createPriceDiv($name,$description,$product_price,$class='',$priceOnly=false,$switchSequel=false,$quantity = 1.0,$forceNoLabel=false){
		...
		if(!empty($this->_priceConfig[$name][0])){
			...
			if(!$switchSequel){
				return '<div class="Price'.($class=='' ? $name : $class).'" style="display : '.$vis.';" >'.$descr.'<span class="Price'.($class=='' ? $name : $class).'" >'.$priceFormatted.'</span></div>';
			} else {
				return '<div class="Price'.($class=='' ? $name : $class).'" style="display : '.$vis.';" ><span class="Price'.($class=='' ? $name : $class).'" >'.$priceFormatted.'</span>'.$descr.'</div>';
			}
		}

...
class VirtueMartModelUser extends VmModel {
    ...
    public function store(&$data) {
        ...
        $a=JRequest::checkToken(); 
        //if(!$a) {            var_dump($_POST); jexit('Invalid Token, while trying to save user'); }
     ...
     */
    function storeAddress(&$data,&$msg=null) {
        ...
        if ($data['address_type'] == 'BT') {
            ...
            if (!$this->validateUserData((array) $data, 'BT',$msg)) {
                return false;
            }
        ...
        if ($data['address_type'] == 'ST' or isset($data['shipto_virtuemart_userinfo_id'])) {
            ...
            if (!$this->validateUserData((array) $dataST, 'ST',$msg)) {
                return false;
            }
     ...
     */
    public function validateUserData($data, $type = 'BT',&$msg=null) {
        ...
        $skips=array('address_type','password','password2');
        $this->change_pass= 0;
        $user = JFactory::getUser();
        if($user->id != 0){
            if(isset($data['shange_pass']) && $data['shange_pass'][0]==1)
                $this->change_pass = 1;
            if(JRequest::getVar('password',null)===null) { 
                $msg['password']=JText::sprintf('COM_VIRTUEMART_MISSING_VALUE_FOR_FIELD', JText::_('Пароль'));
                $msg['password']=JText::sprintf('COM_VIRTUEMART_MISSING_VALUE_FOR_FIELD', JText::_('Повторить пароль'));
            }
        }
        ...
        $neededFields = $userFieldsModel->getUserFields(
                ...
                , $skips);//array('delimiter_userinfo', 'name', 'username', 'password', 'password2', 'address_type_name', 'address_type', 'user_is_vendor', 'agreed'));
        ...
        foreach ($neededFields as $field) {
            ...
            if ($field->required && empty($data[$field->name]) && $field->name != 'virtuemart_state_id') {
                ...
                if ($i > 3 && $type == 'BT') {
                    ...
                    $msg[$field->name]=JText::sprintf('COM_VIRTUEMART_MISSING_VALUE_FOR_FIELD', JText::_($field->title));
        ...
        foreach ($data as $k => $v) {
                $m='';
                if(!$this->validateUserField($k,$v,$m)){
                    $msg[$k]=$m;
                    $return = false;
                }
            }
    ...
    function validateUserField($key, $val, &$msg){
        $db=&JFactory::getDbo();
        $user = JFactory::getUser();
        switch ($key){
            case 'current_password':
                if($this->change_pass != 1)
                    break;
                list($pass,$salt)=explode(':',$user->password);
                if(JUserHelper::getCryptedPassword($val,$salt) !=$pass)
                {
            ...
            case 'email':
                ...
                $q= 'SELECT COUNT(*) FROM #__users WHERE email="'.mysql_escape_string($val).'"';
                if($user->id > 0) $q.=' AND id<>'.$user->id; 
                $db->setQuery($q);
                if($db->loadResult()>0) {
                    $msg= "Пользователь с таким e-mail уже зарегистрирован";
                    return false;
                }
                break;
            ...
            case 'phone_1':
                ...
                $q= 'SELECT COUNT(*) FROM #__virtuemart_userinfos WHERE phone_1="'.mysql_escape_string($val).'"';
                if($user->id > 0) $q.=' AND virtuemart_user_id<>'.$user->id; 
                $db->setQuery($q);
                if($db->loadResult()>0) {
                    $msg= "Пользователь с таким номером телефона уже зарегистрирован";
                    return false;
                }
                break;
            case 'friend':
                if(empty($val)) break;
                $db=&JFactory::getDbo();
                $phoneNo=  str_replace(array('(',')','-','+'), array('','','',''), $val);
                $db->setQuery('SELECT COUNT(*) FROM #__virtuemart_userinfos WHERE phone_1="'.mysql_escape_string($phoneNo).'"');
                $isExists = $db->loadResult();
                if($isExists == 0){
                    $msg='Клиента с таким номером телефона не существует';
                    return false;
                }
                break;
        }
     ...
     * @author Max Milbers
     */
    function getUserInfoInUserFields($layoutName, $type, $uid, $cart = true, $isVendor = false,$skips=array()) {

        // 		if(!class_exists('VirtueMartModelUserfields')) require(JPATH_VM_ADMINISTRATOR.DS.'models'.DS.'userfields.php' );
        // 		$userFieldsModel = new VirtuemartModelUserfields();
        $userFieldsModel = VmModel::getModel('userfields');
        $prepareUserFields = $userFieldsModel->getUserFieldsFor($layoutName, $type, $uid,$skips);
         ...
         */
        if(!class_exists(VirtueMartCart)){
            require_once JPATH_VM_SITE.'/helpers/cart.php';
        }
        $cart=VirtueMartCart::getCart();
        if(!empty($cart->{$type})){
            $JUser = $cart->{$type};
        }
        elseif (!empty($this->_data->JUser)) {
...
// 			vmdebug('$data',$data);
            ...
            if ($data->virtuemart_user_id !== 0 and !$isVendor) {
                ...
                if (!class_exists('Permissions'))
                    require(JPATH_VM_ADMINISTRATOR . DS . 'helpers' . DS . 'permissions.php');
                if (!Permissions::getInstance()->check("admin")) {
                    if ($data->virtuemart_userinfo_id != $this->_id) {
...
// 		vmdebug('getUserAddressList query '.$_q);
    ...
    function getSuperAdminCount() {
        $this->_db->setQuery('SELECT COUNT(id) FROM #__users LEFT JOIN #__user_usergroup_map ON #__users.id=#__user_usergroup_map.user_id'
                . ' WHERE group_id = ' . __SUPER_ADMIN_GID . ' AND block = 0');
        return ($this->_db->loadResult());
    }

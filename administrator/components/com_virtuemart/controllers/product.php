...
// 			vmdebug('$redirect '.$redirect);
	...
	public function resave_all()
	{
				if (!class_exists ('calculationHelper')) {
					require(JPATH_VM_ADMINISTRATOR . DS . 'helpers' . DS . 'calculationh.php');
				}
		$db=& JFactory::getDBO();
		$db->setQuery('SELECT `virtuemart_product_id` as `id` FROM `#__virtuemart_products`');
		$allIDs=$db->loadObjectList();
		$model=VmModel::getModel('product');
		foreach($allIDs as $id){
			$product=$model->getProductSingle($id->id, false);
			$db->setQuery('SELECT * FROM `#__virtuemart_product_prices` WHERE `virtuemart_product_id`='.$id->id);
			$allPrices=$db->loadAssocList();
			foreach ($allPrices as $pricesToStore){
				...
				$mainframe = Jfactory::getApplication();
		$mainframe->redirect('index.php?option=com_virtuemart&view=product','Продукты сохранены успешно');
	}

...
class NewslettersModelNewsletter extends JModel {
    ...
    function getData() {
        $db=&JFactory::getDbo();
        $db->setQuery('SELECT id,`name` FROM #__jinc_newsletter_params WHERE parent = 0');

        $params= $db->loadAssocList();

        foreach ($params as &$param){
            $db->setQuery('SELECT id,`name`,first_index FROM #__jinc_newsletter_params WHERE parent = '.$param['id']);
            $param['children']=$db->loadAssocList();
        }
    ...
    function registerNewsletter($name){
        $db = JFactory::getDbo();
        $db->setQuery('SELECT * FROM #__jinc_newsletter ORDER BY id LIMIT 1');
        $prototype = $db->loadAssoc();
        unset($prototype['id']);
        $prototype['name'] = $name;
        $prototype['created']=date('Y-m-d H:i:s');
        $prototype = (object)$prototype;
        $db->insertObject('#__jinc_newsletter',$prototype);
        return $db->insertid();
    }

    function getNewsletterId(){
        jincimport('utility.BitsetUtil');
        $bitset = new BitsetUtil();
        $params = JRequest::getVar('param',array(),'POST','array');
        $db = JFactory::getDbo();
        $db->setQuery('SELECT MAX(last_index) FROM #__jinc_newsletter_params');
        $len = $db->loadResult();
        $bitset->initFromArray($params,$len);
        $newsletterName = $bitset->toHexStr();
        $db->setQuery('SELECT COUNT(*) FROM #__jinc_newsletter WHERE name='.$newsletterName);
        $count = $db->loadResult();
        if($count == 0) {
           return $this->registerNewsletter($newsletterName);
        }
        $db->setQuery('SELECT id FROM #__jinc_newsletter WHERE name='.$newsletterName);
    ...
    function unsubscribe($id, $subscriber_info) {
        ...
        $suspended = $newsletter->get('suspended');
        if ($suspended == SUSPEND_UNSUBSCRIPTION || $suspended == SUSPEND_BOTH) {
            $this->setError('COM_JINC_ERR052');
            return false;
        }        
        ...
        if ($newsletter->getType() == NEWSLETTER_PUBLIC_NEWS) {
            $this->setState('message', JText::_('COM_JINC_INF004'));
        } else {
            $this->setState('message', JText::_('COM_JINC_INF005'));
        }
        return true;
    }

...
class VirtuemartViewCategory extends VmView {
	...
	public function display($tpl = null) {
		...
		if($category->parents > 1){
			$parent_category=$categoryModel->getCategory(
				$category->parents[count($category->parents)-2]->virtuemart_category_id);
		}
		else $parent_category=null;
		$this->assignRef('parent_category',$parent_category);
		//var_dump($category);
...
/*		if(empty($category->virtuemart_vendor_id) && $search == null ) {
	   ...
	   $this->assignRef('category', $category);
		...
		foreach($products as $product){
              ...
              $product->displayPlugins=$this->getCategoryPlugins($product);
	   ...
	   if ($category->metadesc) {
	    ...
	    shopFunctionsF::setVmTemplate($this,$category->category_template,0,$category->category_layout);
            ...
            $filters = array('price','country','age',
                            'type','gender','brend');
            $needShowAll=false;
            foreach ($filters as $fil){
                if(JRequest::getVar($fil,0)!=0){
                    $needShowAll=true;
                    break;
                }
                
            }
            $this->assignRef('needShowAll', $needShowAll);
            $showAllLink=JRoute::_('index.php?option=com_virtuemart&view=category&virtuemart_category_id='.JRequest::getVar('virtuemart_category_id',0,'get','int').
                        '&Itemid='.JRequest::getVar('Itemid',0,'get','int'));
            $this->assignRef('show_all_link', $showAllLink);

	...
	public function display($tpl = null) {
	  ...
	  if($layoutName=='edit_coupon'){
		...
		$this->assignRef('checkoutAdvertise', $checkoutAdvertise);
                $this->assignRef('productModel', VmModel::getModel ('product'));
		// @max: quicknirty
...
// 		vmdebug('my cart ',$cart);
        ...
        private function getUserFields($skips=array()){
            $userModel=  VmModel::getModel('user');
            $user= $userModel->getCurrentUser();
            //$user=  &JFactory::getUser();
            $skips = array('region','shange_pass','current_password');
            if($user->id != 0){
                $skips[] = 'password';
                $skips[] = 'password2';
            }
            return $userModel->getUserInfoInUserFields('', 'BT',0,$cart = true, $isVendor = false,$skips
                );
        }
...
// 		$this->assignRef('currencyDisplay',$currencyDisplay);
	...
	private function checkShipmentMethodsConfigured() {
        ...
        public function getPopupMessage(&$cart)
        {
            ob_start();
            $_cart=$cart;
            $prodModel=$this->getModel('product');
            require(JPATH_VM_SITE.DS.'views/cart/tmpl/popup.php');
            $res=  ob_get_clean();
            return $res;
        }

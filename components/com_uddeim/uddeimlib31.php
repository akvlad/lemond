...
function uddeIMgetOffset() {
	...
	return $config->get('config.offset');  
}

function uddeIMgetLocale() {
	$config =& JFactory::getConfig();
	return $config->get('config.locale');  
}
...
function uddeIMgetMailFrom() {
	$config =& JFactory::getConfig();
	return $config->get('config.mailfrom');
}

function uddeIMgetMetaDesc() {
	$config =& JFactory::getConfig();
	return $config->get('config.MetaDesc');  
}

function uddeIMgetMetaKeys() {
	$config =& JFactory::getConfig();
	return $config->get('config.MetaKeys');  
}
...
function uddeIMgetDBprefix() {
	$config =& JFactory::getConfig();
	return $config->get('config.dbprefix');  
}
...
function uddeIMisWritable($file, $forcenoftp=false) {
	...
	if (class_exists('JFactory')) {		// Joomla 1.5?
		$config =& JFactory::getConfig();
		$options = array(
			'enabled'	=> $config->get('config.ftp_enable'),
			'host'		=> $config->get('config.ftp_host'),
			'port'		=> $config->get('config.ftp_port'),
			'user'		=> $config->get('config.ftp_user'),
			'pass'		=> $config->get('config.ftp_pass'),
			'root'		=> $config->get('config.ftp_root'),
		);
	}
	if ($forcenoftp)
...
function uddeIMisFtpLayer() {
	if (class_exists('JFactory')) {
		$config =& JFactory::getConfig();
		if ($config->get('config.ftp_enable'))
			return true;
	}
	return false;
}
